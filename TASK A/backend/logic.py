class Exercise:
    """
    Defines the pool of excercises
    """
...
exercises = [
    ...
    Exercise(4, "Childs Pose", ["Floor space"], 120, 3),
...
def filter_exercises(exercises, available_implements, available_time):
    """
    Filters the excercises
    """
    
    available_implements = set(available_implements) | {"None"}
    filtered_exercises = [
        exercise
        for exercise in exercises
        if any(implement.lower() in [i.lower() for i in available_implements] for implement 
               in exercise.implements)]

    filtered_exercises.sort(key=lambda x: x.priority)
    selected_exercises = []
    total_time = 0
